...
export async function POST(request: NextRequest) {
  ...
  const { name, description = '', starts_at, ends_at, currency_name = 'Event-Münzen', banner_color = '#D4A84B' } = body;
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO seasonal_events (name, description, starts_at, ends_at, currency_name, banner_color)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [name.trim(), description, start.toISOString(), end.toISOString(), currency_name, banner_color],
    );
